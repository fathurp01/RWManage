...
export default function RtAuditLogsPage() {
  ...
  const loadRows = useCallback(async (customOffset?: number) => {
    ...
    try {
      const res = await auditLogClient.listForRt({
    ...
    <main className="flex flex-1 flex-col gap-6">
      ...
      <header className="space-y-2">
        <div className="flex items-center gap-2">
          <Badge variant="rt">System Logs</Badge>
        ...
        <CardContent className="px-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="flex items-end">
            {isFilterActive ? (
              ...
              <Button
                className="h-11 w-full gap-2 rounded-xl text-sm font-bold shadow-md bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700 text-white shadow-cyan-500/10 hover:shadow-lg transition-all"
                onClick={() => {
                  setOffset(0);
                  loadRows(0);
                }}
