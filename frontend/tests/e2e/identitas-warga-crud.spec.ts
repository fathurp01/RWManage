...
test('identitas warga create -> list -> delete (API)', async ({ request }) => {
  ...
  const createRes = await request.post('http://localhost:3000/api/rw/identitas-warga', { headers, data: payload })
  ...
  // list
  const listRes = await request.get('http://localhost:3000/api/rw/identitas-warga', { headers, params: { warga_id: firstWarga.id } })
  const listJson = await listRes.json()
  const found = (listJson?.data ?? []).find((x: any) => x.id === created.id)
  if (!found) throw new Error('Created identitas not found')

  // delete
  const delRes = await request.delete(`http://localhost:3000/api/rw/identitas-warga/${created.id}`, { headers })
