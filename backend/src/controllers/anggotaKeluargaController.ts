...
export const updateAnggotaKeluarga = async (
  ...
  try {
    ...
    if (
      (req.user.role === Role.RW && anggota.warga.blok_wilayah.wilayah_rw.user_id !== req.user.id) ||
      (req.user.role === Role.RT && anggota.warga.blok_wilayah_id !== req.user.blok_wilayah_id)
    ) {
      res.status(403).json({
        success: false,
